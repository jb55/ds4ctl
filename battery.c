...
int color_path(const char *device, const char *color, char *path)
{
	...
	snprintf(filename, sizeof(filename), "%s/device/device/leds/*:%s",
		 device, color);
	...
	if (pglob.gl_pathc > 1) {
		snprintf(err, sizeof(err), "got more than one glob in '%s'",
			 filename);
		globfree(&pglob);
		return 0;
	}
...
int write_color(const char *device, const char *color, int val)
{
	...
	if (!color_path(device, color, path))
		return 0;
	...
	if (!fprintf(f, "%d", val)) {
		snprintf(err, sizeof(err),
			 "could not write brightness value to file '%s'",
			 filename);
		fclose(f);
		return 0;
	}
...
int read_color(const char *device, const char *color, int *val)
{
	...
	if (!fscanf(f, "%d", val)) {
		sprintf(err, "could not read brightness value from file '%s'",
			filename);
		fclose(f);
		return 0;
	}
...
int write_trigger(const char *device, const char *trigger)
{
	...
	static char path[256];
	static char filename[512];
	if (!color_path(device, "global", path))
		return 0;
...
int read_capacity(const char *device, int *capacity)
{
	...
	snprintf(filename, sizeof(filename) - 1,
		 "%s/device/device/power_supply/*/capacity", device);
	glob(filename, 0, 0, &pglob);
...
{
	if (!read_capacity(device, &controller->capacity))
		return 0;
	if (!read_color(device, "red", &controller->r))
		return 0;
	if (!read_color(device, "green", &controller->g))
		return 0;
	if (!read_color(device, "blue", &controller->b))
		return 0;
	return 1;
}
	...
	for (size_t i = 0; i < pglob.gl_pathc; ++i) {
		...
		if (!read_controller(device, &controller)) {
			...
			if (controller.capacity < 25) {
				if (!write_color(device, "red", 10))
					fatal();
				if (!write_color(device, "green", 0))
					fatal();
				if (!write_color(device, "blue", 0))
					fatal();
			} else if (controller.capacity < 50) {
				if (!write_color(device, "red", 10))
					fatal();
				if (!write_color(device, "green", 5))
					fatal();
				if (!write_color(device, "blue", 0))
					fatal();
				...
				if (!write_color(device, "red", 10))
					fatal();
				if (!write_color(device, "green", 10))
					fatal();
				if (!write_color(device, "blue", 0))
					fatal();
			} else if (controller.capacity < 100) {
				if (!write_color(device, "red", 0))
					fatal();
				if (!write_color(device, "green", 10))
					fatal();
				if (!write_color(device, "blue", 0))
